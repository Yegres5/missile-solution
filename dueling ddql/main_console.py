...
if __name__ == '__main__':
    ...
    agent = Agent(gamma=0.99, epsilon=1, lr=1e-4,
    ...
    for i in range(num_games):
        ...
        while not done:
            ...
            agent.store_transition(observation, action,
                                   reward, observation_, int(done))
            loss_history.append(agent.learn())
    ...
    with open('score_loss.npy', 'wb') as f:
        np.save(f, scores)
    ...
    # with open('new_version_3l/scores_loss.npy', 'rb') as f:
    #     a = np.load(f)
    #     b = np.load(f)